...
# Veriyi yükle
adult_census = pd.read_csv("Dataset-ML(1).csv")

# Sadece istediğiniz sütunları seçin
selected_columns = ["Cinsiyet", "Doğum Yılı", "Baba Mesleği", "Anne Mesleği", "Evde Bilgisayar"]
...
# Test verisi histogramları
plt.figure(figsize=(12, 6))
for i, column in enumerate(data_test.columns):
    plt.subplot(2, 3, i + 1)
    sns.histplot(data_test[column], kde=True)
    plt.title(column)
plt.tight_layout()
plt.show()
...
# Test verisi violin plotları
plt.figure(figsize=(12, 6))
for i, column in enumerate(data_test.columns):
    plt.subplot(2, 3, i + 1)
    sns.violinplot(x=target_test, y=data_test[column])
    plt.title(column)
plt.tight_layout()
plt.show()
...
# Test verisi box plotları
plt.figure(figsize=(12, 6))
for i, column in enumerate(data_test.columns):
    plt.subplot(2, 3, i + 1)
    sns.boxplot(x=target_test, y=data_test[column])
    plt.title(column)
plt.tight_layout()
plt.show()
